...
(function initBg() {
  ...
  // Mouse position for parallax (normalized 0-1, lerped)
  let mx = 0.5, tx = 0.5;

  // Deterministic stars seeded with index
  ...
  const LAYERS = [
    {
      yFrac: 0.54, amp: 0.21, par: 0.006,
      fill: 'rgba(42,42,42,0.97)',
      freq: [[1.3, 0.4, 0.50], [2.4, 1.9, 0.30], [4.2, 3.3, 0.16], [0.7, 0.8, 0.40]],
    },
    {
      yFrac: 0.60, amp: 0.28, par: 0.014,
      fill: 'rgba(24,24,24,1)',
      freq: [[1.0, 1.6, 0.55], [2.2, 0.3, 0.37], [3.8, 3.0, 0.21], [1.5, 2.3, 0.28]],
    },
    {
      yFrac: 0.66, amp: 0.36, par: 0.024,
      fill: 'rgba(13,13,13,1)',
      freq: [[0.8, 2.9, 0.63], [1.8, 1.2, 0.41], [3.4, 0.6, 0.27], [5.1, 4.3, 0.13]],
    },
    {
      yFrac: 0.73, amp: 0.45, par: 0.038,
      fill: 'rgba(3,3,3,1)',
      freq: [[0.6, 3.6, 0.70], [1.4, 2.1, 0.47], [2.7, 1.4, 0.31], [4.3, 0.9, 0.17]],
    },
  ];

  function profileY(xn, freqs) {
    let v = 0;
    freqs.forEach(([f, ph, a]) => {
      v += Math.max(0, Math.sin(xn * Math.PI * f + ph)) * a;
    });
    return v;
  }
  ...
  function drawLayer(layer, parallaxX) {
    ...
    const ox    = parallaxX * layer.par * w;
  ...
  function frame() {
    ...
    sky.addColorStop(0.40, '#0b0b0b');
    sky.addColorStop(0.60, '#131313');
    sky.addColorStop(1,    '#1a1a1a');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, w, h);

    const gx = w * (0.62 + px * 0.04);
    const gy = h * 0.38;
    ...
    STARS.forEach(s => {
      ctx.beginPath();
      ctx.arc(s.x * w + px * 0.004 * w, s.y * h, s.r, 0, Math.PI * 2);
...
// ── STORAGE ──────────────────────────────────────────────────
function save() {
  localStorage.setItem('nyx_h', JSON.stringify(AS.map(a => ({id:a.id,qty:a.qty,buy:a.buy,manual:a.manual}))));
}
function load() {
  try {
    JSON.parse(localStorage.getItem('nyx_h') || '[]').forEach(s => {
      const a = AS.find(x => x.id === s.id);
      if (a) { a.qty = s.qty||0; a.buy = s.buy||0; if (s.manual != null) a.manual = s.manual; }
    });
  } catch(e) {}
}
...
// Budget
const BUD_KEY = 'nyx_budget';
function saveBudget(obj) { localStorage.setItem(BUD_KEY, JSON.stringify(obj)); }
function loadBudget() {
  try { return JSON.parse(localStorage.getItem(BUD_KEY) || 'null'); } catch(e) { return null; }
}

// Planner
const PLAN_KEY = 'nyx_plan';
function savePlan(day, blocks) { localStorage.setItem(PLAN_KEY + '_' + day, JSON.stringify(blocks)); }
function loadPlan(day) {
  try { return JSON.parse(localStorage.getItem(PLAN_KEY + '_' + day) || 'null'); } catch(e) { return null; }
}
...
async function fetchYahoo(a) {
  ...
  try {
    ...
    if (price && !isNaN(price)) {
      PX[a.id] = { p: price, ch: prev ? (price - prev) / prev * 100 : 0 };
    }
...
async function fetchStooq(a) {
  ...
  try {
    ...
    const prevClose = prev ? parseFloat(prev[4]) : close;
...
// ── INVEST SIGNAL (ETH 7-day trend analysis) ─────────────────
async function fetchInvestSignal() {
  try {
    ...
    if (trend7d > 5 && vol < 4) {
      score = 2; label = 'safe'; icon = '◎';
      note = `ETH +${trend7d.toFixed(1)}% over 7d, volatility low (${vol.toFixed(1)}%/day). Momentum favors entry.`;
    } else if (trend7d < -8 || vol > 7) {
      score = 0; label = 'risky'; icon = '⚠';
      note = `ETH ${trend7d.toFixed(1)}% over 7d, volatility high (${vol.toFixed(1)}%/day). Consider waiting.`;
    } else {
      score = 1; label = 'caution'; icon = '◐';
      note = `ETH ${trend7d > 0 ? '+' : ''}${trend7d.toFixed(1)}% over 7d, vol ${vol.toFixed(1)}%/day. Mixed signals, no strong conviction.`;
    }
...
function renderFearGreed() {
  ...
  const color = value <= 25 ? 'rgba(255,80,80,0.8)'
              : value <= 45 ? 'rgba(255,150,80,0.8)'
              : value <= 55 ? 'rgba(255,255,255,0.5)'
              : value <= 75 ? 'rgba(140,255,170,0.85)'
...
async function fetchEthNews() {
  ...
  for (const feed of feeds) {
    try {
      ...
      ethNews = matched.map(i => ({
        title:  i.title?.trim() || '',
        source: feed.source,
        time:   new Date(i.pubDate || 0).getTime() / 1000,
      }));
      return;
...
function setChartRange(assetId, range) {
  chartRanges[assetId] = range;
  updateChartButtons(assetId);
  renderInvest();
}
...
function renderPortfolio() {
  ...
  if (cel) {
    cel.textContent = T > 0 ? fp(wch) + ' today' : '—';
    cel.className = 'total-ch ' + (wch >= 0 ? 'up' : 'dn');
  }
  ...
  list.innerHTML = AS.map(a => {
    ...
    const vStr = a.qty > 0 ? fe(v) : '—';
    const uStr = a.qty > 0 ? fn(a.qty) + ' u' : 'add';
    return `<div class="asset" onclick="openM('${a.id}')">
      ...
      <div><div class="a-val">${vStr}</div><div class="a-units">${uStr}</div></div>
    </div>`;
  }).join('');
}
...
function renderInvest() {
  ...
  if (eth) {
    ...
    const pEl = document.getElementById('ethPrice');
    const cEl = document.getElementById('ethCh');
  ...
  if (vwce) {
    ...
    const pEl = document.getElementById('vwcePrice');
    const cEl = document.getElementById('vwceCh');
    if (pEl) pEl.textContent = p > 0 ? fe(p) : '—';
    if (cEl) { cEl.textContent = p > 0 ? fp(ch) : '—'; cEl.className = 'ipc-ch ' + (ch >= 0 ? 'up' : 'dn'); }
  }
  ...
  const noteEl = document.getElementById('signalNote');
  if (sig && verdictEl && noteEl) {
    verdictEl.className = `signal-verdict ${sig.label}`;
    verdictEl.textContent = sig.label.toUpperCase();
    noteEl.textContent = sig.note;
  }

  updateChartButtons('eth');
  updateChartButtons('vwce');
...
// ── BUDGET ───────────────────────────────────────────────────
let budget = loadBudget() || { paycheck: 0, rent: 30, invest: 20, fun: 20 };
...
function updateSliderAmt(cat) {
  ...
  const amt = (pay * pct / 100).toFixed(2);
  budget[cat] = pct;
  amtEl.textContent = pay > 0 ? `${pct}%  ·  ${fe(amt, 0)}` : `${pct}%`;
  updateRemaining();
}
...
function updateRemaining() {
  ...
  if (el) {
    el.textContent = fe(rem, 0);
    el.className = 'b-rem-val' + (rem < 0 ? ' over' : '');
  }
  saveBudget(budget);
}

function setPaycheck() {
  const val = parseFloat(document.getElementById('paycheckInput')?.value) || 0;
  budget.paycheck = val;
...
function normalizeBlock(b) {
  if (b && typeof b.from === 'string' && typeof b.to === 'string') {
    return { from: b.from, to: b.to, text: String(b.text || '') };
  }
  return null;
}

function loadOrInitPlan() {
  const key = todayKey();
  savePlan(key, []);
  return [];
}

let planBlocks = loadOrInitPlan();

function renderPlanner() {
  const now = new Date();
  const dEl = document.getElementById('dateBig');
  const dyEl = document.getElementById('dateDay');
  ...
  container.innerHTML = planBlocks.map((b, i) => `
    <div class="time-block" id="tb_${i}">
      <input class="tb-input tb-time-input" type="text"
        placeholder="FROM"
        value="${escAttr(b.from)}"
        onchange="updateBlock(${i},'from',this.value)"
        oninput="updateBlock(${i},'from',this.value)">
      <input class="tb-input tb-time-input" type="text"
        placeholder="TO"
        value="${escAttr(b.to)}"
        onchange="updateBlock(${i},'to',this.value)"
        oninput="updateBlock(${i},'to',this.value)">
      <input class="tb-input tb-desc-input" type="text"
        placeholder="DESCRIPTION"
        value="${escAttr(b.text)}"
        onchange="updateBlock(${i},'text',this.value)"
        oninput="updateBlock(${i},'text',this.value)">
    </div>
  `).join('');
}
